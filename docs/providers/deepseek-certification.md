# VS Brain — Provider Certification Dossier: DeepSeek
...
Current purpose:
- prepare DeepSeek as a post-baseline provider candidate
- define pass/fail checklist before any runtime enablement
- prevent premature addition into `CERTIFIED_PROVIDERS`

## Current known status

- Host mapping already anticipated in code path: `chat.deepseek.com`
- DeepSeek is **not** production-certified
- DeepSeek is **not** allowed into production auto-loop
...
- Planned host: `chat.deepseek.com`
- Current status: host path is anticipated conceptually, but DeepSeek is not active in production gate
- Needed evidence:
  - live DeepSeek tab URL example
  - mapping verification log

### Gate 2 — Latest response extraction
- Current status: no DeepSeek-certified selector set in current hardened runtime
- Needed evidence:
...
### Gate 3 — Prompt fill
- Current status: no DeepSeek-certified composer path
- Needed evidence:
  - input selector path
...
### Gate 4 — Send action
- Current status: no DeepSeek-certified send path
- Needed evidence:
  - send selector path
...
- Current status: not validated
- Needed evidence:
  - finalize prompt sent on DeepSeek
...
- DeepSeek DOM/UI may differ materially from ChatGPT/Gemini assumptions
- Selector shape may be less stable than host mapping suggests
- Finalize flow may fail even if basic relay appears to work
- Premature enablement could create hidden drift across providers

## Minimum live validation plan

Run after current 2-provider baseline is accepted.

1. open DeepSeek tab and verify provider recognition
2. verify latest response extraction on long reply
3. verify prompt fill into DeepSeek composer
4. verify send action
5. verify new response detection
6. verify relay in one pairwise run involving DeepSeek
...
DeepSeek may be moved into `CERTIFIED_PROVIDERS` only when:
1. this dossier is updated with real evidence
2. all 8 gates are PASS
3. owner accepts loop stability impact
4. current certified baseline remains stable after DeepSeek enablement
