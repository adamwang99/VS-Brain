...
function buildFinalMarkdown(source, lang = getLang()) {
  ...
  if (lang === 'en') {
    return `# CrossCritic Final Unified Report
...
Generated by CrossCritic v0.5.1 from the latest provider response.
`;
  }
  return `# CrossCritic - Báo cáo thống nhất cuối
...
Tạo bởi CrossCritic v0.5.1 từ phản hồi mới nhất của provider.
`;
}
...
function buildFinalJson(source, lang = getLang()) {
  ...
  return JSON.stringify({
    ...
    final_report_schema: 'crosscritic.final_report.v1',
    sections: {
...
function buildFinalizePrompt(source, lang = getLang()) {
  if (lang === 'en') return `You are the CrossCritic final-session secretary. Based on the agreed final response below, produce a detailed final unified report with: 1) final decision, 2) complete final answer, 3) resolved critiques, 4) remaining assumptions/limits, 5) next actions, 6) confidence score. Do not repeat debate history.\n\nFINAL RESPONSE:\n${source.content}`;
  return `Bạn là thư ký chốt phiên CrossCritic. Dựa trên phản hồi cuối đã được đồng thuận bên dưới, hãy tạo báo cáo thống nhất cuối thật chi tiết gồm: 1) quyết định cuối, 2) bản trả lời hoàn chỉnh, 3) các phản biện đã xử lý, 4) giả định/giới hạn còn lại, 5) việc cần làm tiếp theo, 6) điểm tin cậy. Không lặp lại lịch sử tranh luận.\n\nPHẢN HỒI CUỐI:\n${source.content}`;
}
...
async function finalizeAndSave() {
  ...
  await downloadText(`${base}-finalize-prompt.md`, buildFinalizePrompt(source, getLang()), 'text/markdown');
