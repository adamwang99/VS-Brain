// CrossCritic Relay v11 — Fixed: Gemini expression syntax, no-nav for Gemini, DeepSeek location.href
...
function nav(id,url){return new Promise(async(ok,rej)=>{
  ...
  w.on('message',d=>{const r=JSON.parse(d+'');if(r.id===1||r.method==='Page.frameNavigated'){w.close();ok(true)}});
  w.on('error',rej);
...
async function gem(msg){
  const t=await findTab('gemini.google.com');
  if(!t){console.log('  [Gemini] missing');return null;}
  console.log('  [Gemini] type on '+t.id.slice(0,8));
  ...
  // Extract — fixed syntax: return"res"})() not return"res")()
...
// ========= DEEPSEEK (use location.href, not nav) =========
async function ds(msg){
  const t=await findTab('deepseek.com');
  if(!t){console.log('  [DeepSeek] missing');return null;}
  console.log('  [DeepSeek] fresh on '+t.id.slice(0,8));

  // Reset to main chat
  await ev(t.id,'window.location.href="https://chat.deepseek.com/"');
  await dly(6000);

  // Type via native setter
  await ev(t.id,'(function(){var ta=document.querySelector(\'textarea[placeholder*="Nhắn"]\');if(!ta)return"no-ta";ta.focus();var ns=Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype,"value").set;ns.call(ta,'+J(msg)+');ta.dispatchEvent(new Event("input",{bubbles:true,cancelable:true}))})()');
  await dly(1500);
  ...
  for(let i=0;i<30;i++){
    ...
    if(r&&r.length>5){
      console.log('  [DeepSeek] res@'+((i*2)+12)+'s: '+r.slice(0,100));
      return '[DeepSeek] '+r;
    }
...
// ========= MAIN =========
(async()=>{
  console.log('===== CROSS CRITIC v11 =====\n');
  const ts=await tabs();
  const c=ts.find(t=>t.url.includes('chatgpt.com'));
  const g=ts.find(t=>t.url.includes('gemini.google.com'));
  const d=ts.find(t=>t.url.includes('deepseek.com'));
  if(!c||!g||!d){console.log('Missing tabs');process.exit(1);}
  console.log('C:'+c.id+'\nG:'+g.id+'\nD:'+d.id);

  const SEED=`Đây là vòng phản biện giữa 3 AI: ChatGPT, Gemini, DeepSeek.
Chủ đề: "AI nên được quản lý bởi chính phủ hay cộng đồng mã nguồn mở?"
Hãy đưa ra quan điểm của bạn trong 2-3 câu, ngắn gọn, có lập luận rõ ràng.`;
  ...
  for(var n=0;n<6;n++){
    ...
    msg=`PHẢN BIỆN: Model trước (${prev}) nói: "${reply.slice(0,200)}"

Hãy đóng vai phản biện — phân tích điểm yếu trong lập luận, hoặc đưa góc nhìn đối lập. Trả lời 2-3 câu ngắn gọn.`;
  }
