...
Generated by CrossCritic v0.6.1 from the latest provider response.
`;
  }
...
Tạo bởi CrossCritic v0.6.1 từ phản hồi mới nhất của provider.
`;
}
...
function buildFinalizePrompt(source, lang = getLang()) {
  if (lang === 'en') return `You are the CrossCritic final-session secretary. Stop critiquing. Stop debating. Produce the final unified blueprint/report from the agreed content below.
...
${source.content}`;
  return `Bạn là thư ký chốt phiên CrossCritic. Dừng phản biện. Dừng tranh luận. Hãy tạo BẢN BLUEPRINT / BÁO CÁO THỐNG NHẤT CUỐI từ nội dung đã đồng thuận bên dưới.
...
async function finalizeAndSave() {
  ...
  const base = `crosscritic/final-blueprint-${safeName(finalSource.platform)}-${Date.now()}`;
